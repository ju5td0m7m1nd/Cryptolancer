...
contract CPL {
  ...
  function assignContract(address _contractor, string _ipfs) public {
    ...
    function winnerReward(address[] jurors, uint[] correctCount, uint[] jurorToken) public onlyBy(Owner) {
        uint total_weight = 0, total_token = 0;
        uint[] weights;

        for (uint i = 0; i < jurors.length; i++){
            total_token = total_token + jurorToken[i];
